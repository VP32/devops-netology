...
Зарегистрировался на sentry.io. В процессе онбординга при регистрации начал создавать пробный проект. Скриншот в Projects:
...
Сгенерировал тестовую ошибку. Нажал для нее Resolved.

Список событий проекта - снял фильтр по unresolved:
...
Алерт настроил, сгенерировал тестовое событие, письмо пришло:
...
В качестве пробного проекта использовал тестовое приложение на Python из предыдущего ДЗ. Поднял его в Докере через docker-compose.

После подключения и работы приложения список Issues стал такой:

![](img/s4.png)

* Код приложения: [run.py](./src/pinger/run.py)
* Манифест [docker-compose.yml](./src/docker-compose.yml)
